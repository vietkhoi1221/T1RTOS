...
class phong:
    ...
    def loaiphong(self):
        with self.con.cursor() as cursor:
            ...
            print("Bên khách sạn của chúng tôi có %d loại phòng sau đây" %self.a)
            for row in cursor:
                self.idlp = row['idloaiPhong']
                self.tenloai = row['moTa']
                # speech.say("Loại thứ {} là {}".format(self.idlp,self.tenloai))
                print("Loại thứ {} là {}".format(self.idlp,self.tenloai))
        # speech.say("Không biết quý khách muốn chọn phòng loại nào")
        print("Không biết quý khách muốn chọn phòng loại nào")
        while True:
            self.aa=0
            ...
            if self.cmd.find("loại")!=-1:
                ...
                with self.con.cursor() as cursor:
                    ...
                    if cursor.rowcount==0:
                        # speech.say("Không có loại phòng này. Quý khách xin vui lòng chọn lại.")
                        print("Không có loại phòng này. Quý khách xin vui lòng chọn lại.")
                    else:
                        self.a=0
                        for row in cursor:
                            ...
                            if self.tt=="trống":
                                ...
                                print("Loại %s có Phòng %s còn trống"%(self.tpe,row['idPhong']))
                                print("Quý khách có muốn chọn loại phòng này không")
                                self.a=1
                                while True:
                                    # self.cmd = speech.phrase().lower()
                                    self.cmd = input("Nhập:")
                                    ...
                                    if self.cmd.find("không")!=-1:
                                        ...
                                        print("vẫn còn sự lựa chọn khác")
                                        break
                    if self.a==0:
                        # speech.say("Rất tiếc phòng loại này đã hết.")
                        print("Rất tiếc phòng loại này đã hết.")
                    elif self.aa==0:
                        # speech.say("Quý khách có thể chọn phòng khác tùy theo sở thích.")
                        print("Quý khách có thể chọn phòng khác tùy theo sở thích.")
                    else:
                        break
            else:
    ...
    def convenient(self):   
        ...
        print("Thêm một thông tin khác " + myweather.completeWeather("Thành phố Hồ Chí Minh"))   
        print("Thời tiết như vậy sẽ rất phù hợp với các dịch vụ sau")   
        with self.con.cursor() as cursor:
            # SQL 
            ...
            print("Có tổng cộng %d dịch vụ" % cursor.rowcount)
            for row in cursor:
                self.IDti = row['idtienIch']
                self.tenti = row['name']
                # speech.say("Dịch vụ số %d là %s " % (self.IDti, self.tenti))
                print("Dịch vụ số %d là %s " % (self.IDti, self.tenti))
        # speech.say("Với nhiều dịch vụ hấp dẫn như vậy, không biết quý khách muốn chọn dịch vụ nào")
        print("Với nhiều dịch vụ hấp dẫn như vậy, không biết quý khách muốn chọn dịch vụ nào")
        ...
        while True:
            ...
            if self.cmd.find("số")!=-1:
                ...
                with self.con.cursor() as cursor:
                    ...
                    cursor.execute(self.sql)
                    if cursor.rowcount==0:
                        # speech.say("Không có dịch vụ này. Quý khách xin vui lòng chọn lại.")
                        print("Không có dịch vụ này. Quý khách xin vui lòng chọn lại.")
                    else:
                        phong.idconv = self.tpeti
                        # speech.say("Quý khách đã chọn dịch vụ này. Xin cảm ơn") 
                        print("Quý khách đã chọn dịch vụ này. Xin cảm ơn") 
                        break
            else:
                # speech.say("Quý khách xin vui lòng nhắc lại.")  
                print("Quý khách xin vui lòng nhắc lại.")  